...
- JSON 匯出，以及帶有預覽與防覆蓋備份的 JSON 匯入。
- 本機資料狀態：總物件數、JSON 估算大小、資料格式版本、核心層資料、執行層資料、封存候選數與封存匯出入口。
- PPV Life OS 百科全書。

資料狀態面板的封存匯出只處理專案與行動。核心層資料指原則、支柱、目的、目標與結果，預設留在主資料中作為長期脈絡。
...
2. 點 `匯入 JSON`。
3. 選擇先前匯出的檔案。
4. 檢查匯入前預覽，確認檔案版本、物件數、有效物件與無效物件數。
5. 點 `先匯出目前資料`，下載目前本機資料作為覆蓋前備份。
6. 確認無誤後，點 `取代目前資料`。

匯入是「取代目前資料」，不是合併。若匯入檔案內有不符合目前 LifeItem v3 格式的物件，預覽會顯示無效數量，確認後只會載入有效物件。有效物件數為 0 時不能匯入。

### 6. 封存完成專案與行動

當完成專案與行動累積太多時，可以在 `資料` 頁的 `封存候選` 區塊點選封存預覽：

1. `預覽完成行動`：列出完成超過 90 天的行動。
2. `預覽專案與行動`：列出完成超過 180 天、底下沒有未完成行動的專案，並帶出完全隸屬於這批封存專案的完成行動。
3. 點 `匯出完整備份`，先保存目前主資料。
4. 點 `匯出封存檔`，保存本次要移出的歷史流水。
5. 確認後點 `從主資料移除封存物件`。

封存檔不是合併匯入格式。第一版若需要回復封存前狀態，請用完整備份透過既有匯入取代流程還原。封存後，週/月/季/年回顧仍只根據主資料即時計算，因此很久以前的完成行動與完成專案細節會從回顧中消失；結果以上物件仍保留。

若某個已完成行動同時連到封存專案與未封存專案，該行動會保留在主資料，並移除指向封存專案的父層連結，避免主資料出現遺失連結。
...
目前正式資料格式是 `version: 3`。這是破壞性 schema 更新，匯入與載入都以目前 LifeItem v3 欄位形狀為準。
...
選擇檔案後，app 會先顯示匯入前預覽，不會立刻覆蓋本機資料。必須先在預覽面板點 `先匯出目前資料`，才可以執行 `取代目前資料`。
