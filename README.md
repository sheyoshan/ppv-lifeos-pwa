...
- 回顧與對齊中心：可切換週、月、季、年，檢查不同時間尺度的執行、成果、策略與人生架構。

點擊首頁的結果卡片會切到 `架構` 頁，並開啟該結果的源頭關係視圖。

「回顧與對齊中心」不是靜態報表，而是首頁內的回顧工作台。週回顧保留既有 Alignment 斷點檢查，可直接查看關係、編輯，或建立缺少的專案/行動。月、季、年回顧則以閱讀與檢查為主，分別聚焦結果交付、目標策略與年度架構校準。

可切換過去週/月/季/年查看歷史期間。第一版歷史回顧是用目前 `LifeItem` 即時計算，不保存回顧心得或 snapshot，因此父層連結與狀態會以目前資料為準。
...
時間視圖採用「層級優先，其次日期」的排序。月視圖固定為 `結果 → 專案 → 行動`，季度路線圖固定為 `目標 → 結果 → 專案`，長期視圖固定為 `目的 → 目標`。同層內再依開始日期、應完成日、狀態與名稱排序；同層重疊時才新增同層 lane，避免不同層級混在同一列。
...
- JSON 匯出與匯入。
- 本機資料狀態：總物件數、JSON 估算大小、資料格式版本、核心層資料、執行層資料與封存候選數。
- PPV Life OS 百科全書。

資料狀態面板只顯示數據，不提供警告或自動清理。核心層資料指原則、支柱、目的、目標與結果；執行層資料指專案與行動。封存候選目前只是統計提示，不會刪除或移動資料。

百科全書是靜態內建內容，不會寫入 `localStorage`，也不會進 JSON 匯出。它提供系統總覽、七大層次、GTD 關係、規劃流程、Alignment、日期節奏、資料維護與封存策略、週回顧與實作範例。
...
2. 看「回顧與對齊中心」的週回顧是否有逾期、未連結或無下一步的斷點。
...
首頁的「回顧與對齊中心」週回顧以本機時間的週一到週日作為範圍。本週完成行動看 `completedDate`，逾期與即將到期看 `dueDate`，硬截止風險看 `deadlineDate`。
...
- 今年的成果是否符合原則？
- 哪些支柱有明確承重，哪些支柱長期沒有推進？
- 哪些目的仍然真實，哪些目的需要重寫？
- 明年應延續、暫停或重新定義哪些目標？

回顧週期採自然週/月/季/年：週一到週日、自然月、自然季 Q1 到 Q4、自然年 1 月 1 日到 12 月 31 日。
