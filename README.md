...
- 可用多選篩選切換行動、專案、結果。
- 可用狀態篩選切換全部、未開始、進行中、完成、逾期與 7 天內應完成。
- 點擊橫條會在月曆下方打開詳情面板，可查看日期、倒數、連結數、buffer 狀態，並可再選擇查看關係或編輯。
- 行動物件可在詳情面板中快速標記完成，或改回進行中。

注意：`deadlineDate` 是截止日期，用來表示絕對不能晚於此日的死線。月曆主橫條仍以 `startDate` 到 `dueDate` 為計畫期間；若 `deadlineDate` 晚於 `dueDate`，會用虛線 buffer 顯示應完成日到截止日之間的緩衝區，並在截止日顯示旗標。若截止日早於應完成日，UI 會顯示高風險提示，但不修改原始資料。
